...
REQUIREMENTS = {
    ...
    "block_polygons": _r(
        "Block boundary polygons", "arcgis", "m_overlay", "real",
        "block_map", "EC 291, EA 196, EB 357 blocks, WGS84, with planted hectares "
        "and palm counts."),
    "planting_year": _r(
        "Planting year per block", "arcgis", "m_overlay / TT", "real",
        "replanting", "2015-2019 across the whole estate."),
    "harvest_short": _r(
        "Block harvest records", "epms_core", "t_oph", "real",
        "yield_choropleth", "EC 19 months, EB eight, EA seven weeks."),
    "grading_log": _r(
    ...
    "harvest_long": _r(
        ...
        "harvest_history", "EPMS holds it. EC's database reaches 19 months; the "
        "synthetic forward months still rest on the first five."),
    "last_harvest": _r(
    ...
    "loose_fruit": _r(
        ...
        "loose_fruit", "38.1M fruits counted on 892,869 EC records, all 291 blocks. "
...
# Features whose output comes out of a fitted model (gis/models) rather than a
# rule or a ratio. The rail marks them so the machine learning is easy to find.
ML = {"crew_productivity", "lagged_forecast", "underperformance_clusters",
      "shrinkage_detection", "stock_outlook", "safety_stock",
      "supplier_lead_time", "tomorrow_outlook"}
...
FEATURES = [
    ...
    _f("grading_by_harvester", "harvesting",
       ...
       "99.82% ripe across EC's 892,869 rows; wet and pest-damaged-old are "
       "all zero and dirty has three non-zero rows. There is nothing to attribute "
    ...
    _f("loose_fruit_recovery", "harvesting",
       ...
       "Runs on EC's own loose_fruits counts: 38.1 million over 19 "
       "months."),
...
# A misspelt id would silently drop the mark from the rail.
assert ML <= {f["id"] for f in FEATURES}, f"unknown ML feature ids: {ML - {f['id'] for f in FEATURES}}"
