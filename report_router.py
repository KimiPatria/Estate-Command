...
from config import engine, ESTATES
from estates import detect_estate_mentions, resolve_scope, strip_estate_mentions
...
class PresetRequest(BaseModel):
    ...
    days: int = 30
    estate: Optional[str] = None  # estate id (e.g. "ba") or "all"/None for every estate
...
def _execute_multi(
    sql: str, estate_ids: list[str], params: dict | None = None, max_rows: int = 1000
    ...
    with ThreadPoolExecutor(max_workers=len(estate_ids)) as pool:
        futures = {
            pool.submit(_execute, sql, params, max_rows, ESTATES[eid]["engine"]): eid
...
def _run_section(
    ...
    """Execute one section's SQL across estate_ids and merge into a single
    ReportSection, tagging rows with _source_estate when more than one
    estate is in scope. Estates that error are dropped, not fatal, and
    reported via estates_failed — same graceful-degradation policy as chat."""
    exec_results = _execute_multi(sql, estate_ids, params, max_rows)
    ...
    if not ok_estates:
        return ReportSection(
            title=title,
            sql=sql.strip(),
            error=exec_results[estate_ids[0]][2],
...
def generate_preset(req: PresetRequest) -> ReportResponse:
    ...
    estate_ids = resolve_scope(req.estate)
    ...
    for section_title, sql in _PRESETS[req.preset]:
        ...
        if not ok:
            ...
            continue
        section = _run_section(section_title, sql, estate_ids, params)
        sections.append(section)
        log.info("[report/preset] %s → %d rows", section_title, len(section.rows))
...
def generate_text_report(req: GenerateReportRequest):
    ...
    query = apply_input_guardrails(query, context="report")

    log.info('[report/generate] query="%s"', query)

    from error_handler import error_response as _err_resp, is_off_domain

    mentioned_estates = detect_estate_mentions(query)
    estate_ids = resolve_scope(req.estate, query)
    # SQL generation only — table retrieval still sees the original query.
    # See estates.strip_estate_mentions for why this is needed.
    sql_query = strip_estate_mentions(query, mentioned_estates) if mentioned_estates else query
    ...
    messages = build_sql_messages(sql_query, table_cards, examples)
    raw_text, provider_used, usage = call_llm(
        messages, max_tokens=600, temperature=0, task="sql_generation"
    )
    log.info(
    ...
    # 6. Execute across the relevant estate(s)
    section = _run_section("Query Results", sql, estate_ids)
    if section.error:
    ...
    log.info("[report/generate] done — %d rows", len(section.rows))
...
def run_saved_template(req: RunSavedRequest) -> ReportResponse:
    ...
    estate_ids = resolve_scope(req.estate)
    section = _run_section(req.name, sql, estate_ids, params)
    log.info("[report/run-saved] done — %d rows, error=%s", len(section.rows), section.error)
...
def run_template_by_id(req: RunTemplateRequest) -> ReportResponse:
    ...
    estate_ids = resolve_scope(req.estate)
    section = _run_section(tpl["name"], sql, estate_ids)
    log.info("[report/run-template] done — %d rows, error=%s", len(section.rows), section.error)
